...
const DEV_STATUS_STYLES: Record<DevStatus, string> = {
  ...
  offline: 'text-muted bg-neutral-500/10 border-neutral-500/20',
}
...
function PasswordInput({ value, onChange, show, onToggle, placeholder }: {
  ...
  return (
    <div className="relative">
      ...
      />
      <button type="button" onClick={onToggle} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted hover:text-secondary transition-colors">
...
function SpecialtiesField({ specialties, onToggle, customSkill, onCustomChange, onAddCustom }: {
  ...
  return (
    <div className="space-y-2">
      <label className="text-xs font-medium text-secondary">Especialidades</label>
          ...
          <button key={s} type="button" onClick={() => onToggle(s)}
            className={['px-3 py-1 text-xs rounded-lg border transition-all',
              specialties.includes(s)
                ...
                : 'bg-surface text-muted border-border hover:text-secondary hover:border-border-strong',
            ].join(' ')}>
            {s}
          </button>
        ))}
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onAddCustom}
          className="px-3 py-2 text-xs text-secondary hover:text-foreground bg-elevated hover:bg-elevated border border-border rounded-xl transition-colors">
          Adicionar
        </button>
      </div>
      {custom.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {custom.map((s) => (
            <span key={s} className="flex items-center gap-1 px-2.5 py-1 text-xs rounded-lg bg-brand/10 text-brand border border-brand/30">
              {s}
              <button type="button" onClick={() => onToggle(s)} className="hover:text-foreground transition-colors ml-0.5">
                <X size={10} />
              </button>
            </span>
          ))}
      ...
      <form onSubmit={handleSubmit}>
        <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
          ...
          <div className="flex items-start gap-2.5 p-3 rounded-xl bg-surface border border-border">
            <Wifi size={14} className="text-muted mt-0.5 shrink-0" />
            <p className="text-xs text-muted leading-relaxed">
              Presença detectada automaticamente — <span className="text-success">disponível</span> quando online sem projetos, <span className="text-yellow-400">ocupado</span> com projetos ativos.
            </p>
          </div>
        ...
        {firebaseError && <p className="px-6 py-3 text-xs text-danger bg-red-400/5 border-t border-red-400/10">{firebaseError}</p>}
      ...
      <form onSubmit={handleSubmit}>
        <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
          <div className="flex items-center gap-3 p-3.5 rounded-xl bg-surface border border-border">
            ...
            <div>
              <p className="text-sm font-medium text-foreground">{name || dev.name}</p>
              <p className="text-xs text-muted">{dev.email}</p>
            </div>
          </div>
          <Field label="Nome completo" required>
      ...
      <div className="relative w-full max-w-sm bg-surface border border-border-strong rounded-2xl shadow-2xl overflow-hidden animate-fade-up">
        <div className="px-6 pt-7 pb-5 flex flex-col items-center text-center gap-4">
          ...
          <div className="space-y-1">
            <h2 className="text-base font-bold text-foreground">Excluir dev?</h2>
            <p className="text-sm text-secondary leading-relaxed">
              O cadastro de <span className="text-foreground font-medium">{dev.name}</span> será removido permanentemente.
            </p>
            <p className="text-xs text-faint mt-1">A conta de autenticação deve ser removida manualmente no Firebase.</p>
          </div>
          {error && <p className="text-xs text-danger">{error}</p>}
        </div>
        <div className="px-6 pb-6 flex gap-3">
          <button onClick={onClose} disabled={loading}
            className="flex-1 py-2.5 text-sm text-secondary hover:text-foreground bg-elevated hover:bg-elevated rounded-xl transition-colors disabled:opacity-40">
            Cancelar
          </button>
...
function ProjectRow({ project }: { project: RichProject }) {
  return (
    <div className="flex items-center gap-3 p-3.5 rounded-xl bg-surface border border-border hover:bg-elevated transition-colors">
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-foreground truncate">{project.product}</p>
        <p className="text-xs text-muted mt-0.5 truncate">{project.customer}</p>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <span className="text-xs text-faint hidden sm:block">{project.date}</span>
      ...
      <div className="px-6 py-5 space-y-5 max-h-[65vh] overflow-y-auto">
        {loading ? (
          ...
          <div className="text-center py-10">
            <Briefcase size={32} className="text-faint mx-auto mb-3" />
            <p className="text-sm text-muted">Nenhum projeto atribuído.</p>
          </div>
        ) : (
          <>
            {active.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs font-semibold text-secondary uppercase tracking-wider">Em andamento</span>
                </div>
            ...
            {finished.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs font-semibold text-secondary uppercase tracking-wider">Concluídos</span>
                </div>
...
function DevCard({ dev, onEdit, onDelete, onViewProjects }: {
  ...
  return (
    <div className="relative rounded-2xl bg-surface border border-border hover:border-border-strong transition-all overflow-hidden flex flex-col items-center text-center px-5 pt-6 pb-4 gap-3">

      {/* Status strip at top */}
      <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-10 h-[3px] rounded-b-full ${statusLine}`} />

      {/* Avatar — circle */}
      <div className="w-16 h-16 rounded-full border-2 border-border-strong overflow-hidden flex items-center justify-center shrink-0 bg-gradient-to-br from-brand/20 to-brand/5">
        {dev.avatar
      ...
      {/* Name + email */}
      <div>
        <p className="text-sm font-semibold text-foreground leading-none">{dev.name}</p>
        <p className="text-[11px] text-faint mt-1 truncate max-w-[160px]">{dev.email}</p>
      </div>

      {/* Status */}
      <div className="flex items-center gap-1.5">
        <OnlineDot status={dev.status} />
        <span className="text-[11px] text-muted">{DEV_STATUS_LABELS[dev.status]}</span>
      </div>
      ...
      {dev.specialty.length > 0 && (
        <p className="text-[11px] text-faint leading-relaxed">
          ...
          {dev.specialty.length > 3 && (
            <span className="text-faint"> +{dev.specialty.length - 3}</span>
          )}
        </p>
      )}
      ...
      <div className="w-full grid grid-cols-2 gap-2 mt-1">
        <div className="rounded-xl bg-elevated py-2.5">
          <p className="text-base font-bold text-foreground leading-none">{dev.activeProjects}</p>
          <p className="text-[10px] text-faint mt-1">ativos</p>
        </div>
        <div className="rounded-xl bg-elevated py-2.5">
          <p className="text-base font-bold text-foreground leading-none">{dev.completedProjects}</p>
          <p className="text-[10px] text-faint mt-1">entregues</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 pt-1">
        <button onClick={() => onViewProjects(dev)} title="Ver projetos"
          className="p-1.5 rounded-lg text-faint hover:text-brand hover:bg-brand/8 transition-all">
          <Briefcase size={13} />
        </button>
        <button onClick={() => onEdit(dev)} title="Editar"
          className="p-1.5 rounded-lg text-faint hover:text-foreground hover:bg-elevated transition-all">
          <Pencil size={13} />
        </button>
        <button onClick={() => onDelete(dev)} title="Excluir"
          className="p-1.5 rounded-lg text-faint hover:text-danger hover:bg-red-400/8 transition-all">
          <Trash2 size={13} />
        </button>
      </div>
    </div>
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-[11px] font-medium text-faint uppercase tracking-widest">Equipe</p>
            <h1 className="text-2xl font-bold text-foreground mt-1">
              Desenvolvedores
              {!pageLoading && (
                <span className="ml-2.5 text-base font-normal text-faint">{devs.length}</span>
              )}
            </h1>
          </div>
          <button
              ...
              <button
                ...
                className={[
                  ...
                  filter === key
                    ...
                    : 'text-faint border-transparent hover:text-secondary',
                ].join(' ')}
              >
                {key !== 'todos' && (
        ...
        {pageLoading ? (
          ...
          <div className="rounded-2xl bg-surface border border-border p-12 text-center">
            <Code2 size={32} className="text-faint mx-auto mb-4" />
